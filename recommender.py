...
# Reuse a session so requests don't pay setup overhead for every poster lookup.
...
try:
    ...
    print("Required files: movies.pkl, embeddings.pkl, vectorizer.pkl, vectors.pkl")
    raise
...
def fetch_poster(title):
    ...
    # If the API key is missing, avoid blocking the page on a request that will fail.
    if not API_KEY:
...
def get_movie_details(title):
    ...
    if clean not in title_list:
        suggestion = get_close_matches(clean, title_list, n=1, cutoff=0.6)
        if suggestion:
            clean = suggestion[0]
        else:
            return None

    row = movies[movies['clean_title'] == clean].iloc[0]

    cast_list = [item.strip() for item in str(row.get('cast_names', '')).split(',') if item.strip()][:8]
    genre_list = [item.strip() for item in str(row.get('genres', '')).split(',') if item.strip()][:6]
    keyword_list = [item.strip() for item in str(row.get('keywords', '')).split(',') if item.strip()][:8]

    overview = str(row.get('overview', '')).strip()
    tagline = str(row.get('tagline', '')).strip()
    if not tagline:
        tagline = "A must-watch pick for movie lovers."

    review_summary = overview if overview else tagline
    if len(review_summary) > 280:
        review_summary = review_summary[:277] + '...'

    return {
        "title": row.get('title', title),
        "poster": fetch_poster(row.get('title', title)),
        "overview": overview or "No summary available yet.",
        "review_summary": review_summary,
        "genres": genre_list,
        "cast": cast_list,
        "director": row.get('director_name', 'Unknown') or 'Unknown',
        "rating": float(row.get('vote_average', 0)) if row.get('vote_average') is not None else 0,
        "vote_count": int(row.get('vote_count', 0)) if row.get('vote_count') is not None else 0,
